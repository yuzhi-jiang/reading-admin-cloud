package com.yefeng.gateway.filter;
...
import com.yefeng.gateway.common.config.SystemPropertiesConfig;
import com.yefeng.gateway.common.utils.JwtUtil;
...
//        System.out.println("白名单:");
        ...
        // 主页接口、图书接口正则匹配
        boolean bookMatch = Pattern.matches("/bookApi/[^\\s]*", path);
        boolean account = Pattern.matches("/accountApi/[^\\s]*", path);


        // 白名单接口、开放接口放行
        if (bookMatch  ||account || whiteList.contains(path)) {
